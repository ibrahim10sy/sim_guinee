...
public class FicheCollecteController {
    ...
    public ResponseEntity<FicheCollecte> updateFiche(@PathVariable int id, @RequestBody FicheCollecte a ){
        return new ResponseEntity<>(aService.update(a, id), HttpStatus.OK);
    }
 
    @GetMapping("/getById/{id}")
    @Operation(summary="Récuperation d'une fiche de collecte")
    public ResponseEntity<FicheCollecte> getById(@PathVariable int id) {
    ...
    public ResponseEntity<List<FicheCollecte>> getAllFicheCollecteByEnquete(@PathVariable int id) {
